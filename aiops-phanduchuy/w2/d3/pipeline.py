...
def process_batch(alerts: list[dict]) -> dict:
    ...
    - clusters: list of clusters with metadata (cluster_id, alert_count, services involved, time range)
    """
    # L1: Correlate
    ...
    for inc_id in similar_incidents_list[:3]:
        ...
        if hist_item:
            ...
            match = next(
                (item for item in similar_retrieved if item["id"] == inc_id), None
            )
        ...
        similar_details.append(
            {"id": inc_id, "similarity": float(sim_score), "summary": summary_text}
        )
